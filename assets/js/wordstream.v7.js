    ...
    window.wordstream = function (svg, data, config) {
        d3.wordstreamLayout = function () {
            ...
            function buildFontScale(data) {
                let topics = Object.keys(data[0].words);//d3.keys(data[0].words);
            ...
            function buildFrequencyScale(data) {
                ...
                d3.map(totalFrequencies, function (d) {
                    let keys = Object.keys(totalFrequencies[0]);//d3.keys(totalFrequencies[0]);
                    let total = 0;
                    keys.forEach(key => {
                        total += d[key];
                    });
            ...
            function angle(boxes) {
                ...
                Object.keys(data[0].words)
                for (let i = 0; i < Object.keys(innerBoxes).length; i++) {   // i: index cua topic
                    let key = Object.keys(innerBoxes)[i];
